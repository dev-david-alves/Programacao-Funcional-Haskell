...
dig2char dig = head $ [letter | (value, letter) <- zip [0 ..] ['0' .. '9'], value == dig]
...
solve (xs, lim) holes hindex
  ...
  | null solved = Nothing
  | otherwise = head solved
  where
    pos = holes !! hindex
    tests = [solve (set xs pos value, lim) holes (hindex + 1) | value <- [0 .. lim], fit (xs, lim) pos value]
    solved = [x | x <- tests, isJust x]
